...
用户问题通常简短且口语化（如"AGV怎么调度"），而文档中的表述更专业冗长（如"多载量自动导引车系统的任务分配与路径规划方法"）。两者在向量空间中距离较远。

#### 实现原理

让 LLM 生成一段假设性答案，其用词更接近文档的表述风格，用假设答案的 embedding 做检索，命中率更高。
...
- 假设答案不需要准确，关键是包含领域术语，拉近与文档的向量距离
